...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .specs-grid { display:grid; grid-template-columns:1fr 1fr; gap:4px 10px; font-size:12px; color:var(--mute); margin:4px 0; padding:10px 0; border-top:1px solid var(--line); border-bottom:1px solid var(--line); }
  .specs-grid b { color:var(--ink); font-weight:600; }
    ...
    <div className="card">
      ...
      <div className="card-body">
        ...
        <div className="specs-grid">
          <span><b>{fmtKm(car.km)}</b></span>
          <span><b>{car.transmision === "automatico" ? "Automático" : "Manual"}</b></span>
          <span>{COMBUSTIBLE_LABEL[car.combustible] || car.combustible}</span>
          <span>{CARROCERIA_LABEL[car.carroceria] || car.carroceria}</span>
          <span>{car.color}</span>
          <span>{car.traccion === "4x4" ? "4x4 / AWD" : "4x2"}</span>
          {car.motor && <span>Motor {car.motor}</span>}
          {car.puertas && <span>{car.puertas} puertas</span>}
        </div>
...
function App() {
  ...
  const filtrados = useMemo(() => cars.filter(c =>
    ...
    (combustible === "todos" || c.combustible === combustible) &&
    (carroceria === "todos" || c.carroceria === carroceria) &&
    c.km >= km[0] && c.km <= km[1] &&
    c.precio >= precio[0] && c.precio <= precio[1]
  ), [cars, condicion, transmision, combustible, carroceria, km, precio]);

  function clearFilters() {
    setCondicion("todos"); setTransmision("todos"); setCombustible("todos"); setCarroceria("todos");
    setKm([0, 350000]); setPrecio([0, 50000]);
  }

  const carroceriasEnUso = useMemo(() => {
    const set = new Set(cars.map(c => c.carroceria).filter(Boolean));
    ...
    <React.Fragment>
      ...
      <div className="catalogo">
        <aside className="filtros">
          ...
          <FilterCard title="Combustible">
            <div className="chips">
              <button className={combustible === "todos" ? "chip on" : "chip"} onClick={() => setCombustible("todos")}>Todos</button>
              {Object.entries(COMBUSTIBLE_LABEL).map(([v, l]) => (
                <button key={v} className={combustible === v ? "chip on" : "chip"} onClick={() => setCombustible(v)}>{l}</button>
              ))}
            </div>
          </FilterCard>

          {carroceriasEnUso.length > 0 && (
            <FilterCard title="Carrocería">
              <div className="chips">
                <button className={carroceria === "todos" ? "chip on" : "chip"} onClick={() => setCarroceria("todos")}>Todas</button>
                {carroceriasEnUso.map(v => (
                  <button key={v} className={carroceria === v ? "chip on" : "chip"} onClick={() => setCarroceria(v)}>{CARROCERIA_LABEL[v] || v}</button>
                ))}
              </div>
            </FilterCard>
          )}
...
git add -A
git commit -m "Panel con menu lateral y ficha tecnica completa; filtros mas prolijos en la pagina" || echo "Nada nuevo para commitear"
git push
echo ""
echo "LISTO."
