...
<link href="https://fonts.googleapis.com/css2?family=Anton&family=Work+Sans:wght@400;500;600;700&family=Space+Mono:wght@700&display=swap" rel="stylesheet">
...
<style>
  :root {
    --paper:#FFFFFF; --alt:#F5F6F4; --ink:#1B2B22; --green:#2C6349; --greenDark:#1E4832;
    --amber:#C98A12; --whats:#22B24C; --line:#E3E1D8; --mute:#6B7268;
  }
  * { box-sizing:border-box; }
  body { margin:0; background:var(--paper); color:var(--ink); font-family:'Work Sans',system-ui,sans-serif; }

  .navbar { display:flex; align-items:center; justify-content:space-between; padding:16px 28px; border-bottom:1px solid var(--line); position:sticky; top:0; background:var(--paper); z-index:10; }
  .navbar .logo { font-family:'Anton',sans-serif; font-size:22px; letter-spacing:0.5px; color:var(--greenDark); }
  .navbar .logo span { color:var(--amber); }

  .hero { background:var(--green); color:#fff; padding:44px 28px; text-align:center; }
  .hero h1 { font-family:'Anton',sans-serif; font-size:clamp(30px,5vw,44px); margin:0 0 8px; letter-spacing:0.5px; }
  .hero p { margin:0; color:#dcE7e0; font-size:15px; }

  .catalogo { display:grid; grid-template-columns:260px 1fr; gap:24px; padding:28px; max-width:1200px; margin:0 auto; }
  .filtros { background:var(--alt); border:1px solid var(--line); border-radius:14px; padding:20px; align-self:start; position:sticky; top:76px; }
  .filtros h2 { margin:0 0 16px; font-size:15px; font-weight:700; }
  ...
  .chip { background:#fff; border:1px solid var(--line); color:var(--ink); padding:6px 12px; border-radius:20px; font-size:13px; cursor:pointer; }
  ...
  .rd-fill { position:absolute; top:12px; height:4px; background:var(--green); border-radius:2px; }
  ...
  .rd-track input[type=range]::-webkit-slider-thumb { -webkit-appearance:none; pointer-events:auto; width:16px; height:16px; border-radius:50%; background:var(--green); border:2px solid #fff; box-shadow:0 0 0 1px var(--line); cursor:pointer; }
  .rd-track input[type=range]::-moz-range-thumb { pointer-events:auto; width:16px; height:16px; border-radius:50%; background:var(--green); border:2px solid #fff; cursor:pointer; }
  ...
  .card { background:#fff; border:1px solid var(--line); border-radius:14px; overflow:hidden; display:flex; flex-direction:column; box-shadow:0 1px 3px rgba(27,43,34,0.05); }
  ...
  .tag { position:absolute; top:10px; left:10px; font-size:11px; font-weight:700; padding:4px 9px; border-radius:6px; letter-spacing:0.5px; }
  ...
  .thumb { width:34px; height:26px; border-radius:5px; border:2px solid transparent; background-size:cover; background-position:center; padding:0; cursor:pointer; }
  .thumb.on { border-color:var(--green); }
  .card-body { padding:12px 16px 16px; display:flex; flex-direction:column; gap:6px; }
  ...
  .desc { font-size:12.5px; color:#495047; margin:2px 0; line-height:1.4; }
  .plate { align-self:flex-start; font-family:'Space Mono',monospace; font-weight:700; font-size:16px; background:#fff; border:2px solid var(--ink); border-radius:6px; padding:4px 10px; margin-top:4px; }
  .btn-whats { display:flex; align-items:center; justify-content:center; gap:8px; background:var(--whats); color:#fff; text-decoration:none; padding:10px; border-radius:8px; font-weight:600; font-size:13.5px; margin-top:6px; }
  ...
  return (
    <div className="card">
      ...
      <div className="card-body">
        ...
        <div className="plate">{fmtMoney(car.precio)}</div>
        <a
...
<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
<style>
  :root { --paper:#FFFFFF; --alt:#F5F6F4; --ink:#1B2B22; --green:#2C6349; --greenDark:#1E4832; --line:#E3E1D8; --mute:#6B7268; }
  * { box-sizing:border-box; }
  ...
  .panel-top h1 { font-family:'Anton',sans-serif; font-size:26px; letter-spacing:0.5px; color:var(--greenDark); margin:0; }
  .panel-top a { color:var(--green); font-size:13px; text-decoration:none; font-weight:600; }
  .settings, .form { background:#fff; border:1px solid var(--line); border-radius:14px; padding:24px; }
  .settings h2, .form h2 { margin-top:0; font-size:16px; }
  ...
  .f input, .f select, .f textarea { width:100%; padding:9px 10px; border-radius:8px; border:1px solid var(--line); background:var(--alt); color:var(--ink); font-size:14px; }
  .thumbs-grid { display:flex; flex-wrap:wrap; gap:10px; }
  .thumb-item { width:80px; height:64px; border-radius:8px; background-size:cover; background-position:center; position:relative; border:1px solid var(--line); }
  ...
  .add-photo { width:80px; height:64px; border-radius:8px; border:1.5px dashed var(--line); display:flex; align-items:center; justify-content:center; font-size:12px; color:var(--mute); cursor:pointer; text-align:center; }
  .form-actions { display:flex; gap:10px; margin-top:8px; }
  .btn { background:var(--green); color:#fff; border:none; padding:10px 20px; border-radius:8px; font-weight:600; cursor:pointer; font-size:14px; }
  .btn.ghost { background:transparent; border:1px solid var(--line); color:var(--ink); }
  .btn.ghost.danger { color:#B3261E; border-color:#e3c6c3; }
  .lista h2 { font-size:16px; }
  .fila { display:flex; justify-content:space-between; align-items:center; padding:12px 16px; background:#fff; border:1px solid var(--line); border-radius:10px; margin-bottom:8px; }
  ...
  .mute { color:var(--mute); }
  .login { max-width:340px; margin:80px auto; background:#fff; border:1px solid var(--line); padding:28px; border-radius:14px; text-align:center; }
  .login p { color:var(--mute); font-size:13px; margin-bottom:16px; }
  .login input { width:100%; padding:10px; border-radius:8px; border:1px solid var(--line); background:var(--alt); color:var(--ink); margin-bottom:12px; }
...
const SITE_URL = "https://tu-pagina.netlify.app";
...
const empty = { marca: "", modelo: "", anio: new Date().getFullYear(), km: 0, precio: 0, condicion: "usado", transmision: "manual", color: "", fotos: [], descripcion: "" };
  ...
  return (
    <div className="panel">
      ...
      <div className="settings">
        <h2>Configuración</h2>
        <div className="f">
          <label>Número de WhatsApp (con código de país, sin + ni espacios. Ej: 5491122334455)</label>
      ...
      <form className="form" onSubmit={submit}>
        ...
        <div className="row">
          ...
          <div className="f small"><label>Año</label><input type="number" value={form.anio} onChange={e => set("anio", Number(e.target.value))} /></div>
        </div>
        <div className="row">
          <div className="f"><label>Kilómetros</label><input type="number" value={form.km} onChange={e => set("km", Number(e.target.value))} /></div>
          <div className="f"><label>Precio (USD)</label><input type="number" value={form.precio} onChange={e => set("precio", Number(e.target.value))} /></div>
        ...
        <div className="f"><label>Fotos</label><PhotoUploader fotos={form.fotos} onChange={(f) => set("fotos", f)} /></div>
        <div className="f"><label>Descripción</label><textarea value={form.descripcion} onChange={e => set("descripcion", e.target.value)} rows={3} /></div>
...
git add -A
git commit -m "Fix: CDN supabase, style tag, version de Babel" || echo "Nada nuevo para commitear"
git push
echo ""
